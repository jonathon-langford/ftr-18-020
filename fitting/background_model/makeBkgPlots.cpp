...
int main(int argc, char* argv[]){
  ...
  string catname;
	...
	// now generate a new dataSet which will just be pseudo-data 
	int Ndata = (int)data->sumEntries();
	//RooDataSet *dataT = (RooDataSet*)data ;//mpdf->generate(RooArgSet(*mgg),NumEvents(Ndata));
	
	RooDataSet *dataT = mpdf->generate(RooArgSet(*mgg),NumEvents(Ndata));
	bf = getBestFitFunction(mpdf,dataT,mcat,!verbose_);
	mcat->setIndex(bf);
	...
	plot->GetXaxis()->SetTitle("m_{#gamma#gamma} (GeV)");
	plot->SetTitle("");
	dataT->plotOn(plot,Binning(80),Invisible());
  ...
  // enf extra bit for ratio plot///
	...
	leg->AddEntry(dataLeg,"Pseudo-data","LEP");
	...
	if (doBands) {
      ...
      double nllBest = getNormTermNll(mgg,dataT,mpdf,mcat,nomBkg,lowedge,upedge);
       ...
       std::cout << "[FOR TABLE] ,"<<flashggCats_[cat]<<","<< mass << ","<<nomBkg_perGeV<<", assuming resolution of " << higgsResolution_ << std::endl;
			...
			if (nomBkg<1.e-5) {
				...
				errHigh1Value = guessNew(mgg,mpdf,mcat,dataT,nomBkg,nllBest,highRange,lowedge,upedge,1.,nllTolerance);
				if (verbose_) cout << "[INFO] "<< "errHigh2" << endl;
				errHigh2Value = guessNew(mgg,mpdf,mcat,dataT,nomBkg,nllBest,highRange,lowedge,upedge,4.,nllTolerance);
			}
			else {
				// error calc algo
				if (verbose_) cout<< "[INFO] " << "errLow1" << endl;
				errLow1Value = guessNew(mgg,mpdf,mcat,dataT,nomBkg,nllBest,lowRange,lowedge,upedge,1.,nllTolerance);
				if (verbose_) cout<< "[INFO] " << "errLow2" << endl;
				errLow2Value = guessNew(mgg,mpdf,mcat,dataT,nomBkg,nllBest,lowRange,lowedge,upedge,4.,nllTolerance);
				if (verbose_) cout<< "[INFO] " << "errHigh1" << endl;
				errHigh1Value = guessNew(mgg,mpdf,mcat,dataT,nomBkg,nllBest,highRange,lowedge,upedge,1.,nllTolerance);
				if (verbose_) cout<< "[INFO] " << "errHigh2" << endl;
				errHigh2Value = guessNew(mgg,mpdf,mcat,dataT,nomBkg,nllBest,highRange,lowedge,upedge,4.,nllTolerance);
        ...
        temp->SetTickx(); temp->SetTicky();
				...
				for (double scanVal=0.9*errLow2Value; scanVal<1.1*errHigh2Value; scanVal+=1){
					double nll = getNormTermNll(mgg,dataT,mpdf,mcat,scanVal,lowedge,upedge)-nllBest;
		...
		outWS->import(*mpdf);
		outWS->import(*data);
		dataT->SetName(Form("%s_pseudodata",data->GetName()));
		outWS->import(*dataT);
  ...
  // enf extra bit for ratio plot///
    canv->SetTickx(); canv->SetTicky();
		...
		if (!unblind) {
			mgg->setRange("unblind_up",135,180);
			mgg->setRange("unblind_down",100,115);
			dataT->plotOn(plot,Binning(80),CutRange("unblind_down,unblind_up"));
		}
		else {
			dataT->plotOn(plot,Binning(80));
		}
	...
	//		if (!w_sig) w_sig = (RooWorkspace*)sigFile->Get("wsig_13TeV");
      ...
      if (w_sig) SignalType=1;
			...
			if (SignalType==1) {
				...
				MH->setVal(mhvalue_);
				sigPDF->plotOn(plot,Normalization(1,RooAbsReal::RelativeExpected),LineColor(kBlue),LineWidth(3));
				sigPDF->plotOn(plot,Normalization(1,RooAbsReal::RelativeExpected),LineColor(kBlue),LineWidth(3),FillColor(38),FillStyle(3001),DrawOption("F"));
				...
				sigPDF->plotOn(plotLC,Normalization(1,RooAbsReal::RelativeExpected),LineColor(kBlue),LineWidth(3));
