...
int main(int argc, char *argv[]) {
  ...
  RooAbsData *datain = (RooAbsData*)fin->Get("toys/toy_asimov");
  ...
  for (int idat=0; idat<datain->numEntries(); ++idat) {
    ...
    //data->add(*dset,datain->weight());
  }
  */
  data = (RooDataSet*)datain;

  std::cout << "Ok we made the dataset " << std::endl ;
  data->Print("V");
  ...
  for (int i=0; i<chan->numTypes(); ++i) {
   ...
   // desc.ReplaceAll("lep","");
    desc.ReplaceAll("datacard_ttH_differential","");
    desc.ReplaceAll("_"," ");
    desc.ReplaceAll("ttHHad datacard ttHHad","");
    desc.ReplaceAll("ttHLep datacard ttHLep","");
    
    desc.ReplaceAll("lep reco0","#splitline{Leptonic}{p_{T}^{#gamma#gamma} < 45 GeV}");
    desc.ReplaceAll("lep reco1","#splitline{Leptonic}{45 #leq p_{T}^{#gamma#gamma} < 80 GeV}");
    desc.ReplaceAll("lep reco2","#splitline{Leptonic}{80 #leq p_{T}^{#gamma#gamma} < 120 GeV}");
    desc.ReplaceAll("lep reco3","#splitline{Leptonic}{120 #leq p_{T}^{#gamma#gamma} < 200 GeV}");
    desc.ReplaceAll("lep reco4","#splitline{Leptonic}{200 #leq p_{T}^{#gamma#gamma} < 250 GeV}");
    desc.ReplaceAll("lep reco5","#splitline{Leptonic}{p_{T}^{#gamma#gamma} > 350 GeV}");

    desc.ReplaceAll("BDT","");
    desc.ReplaceAll("reco0 b","#splitline{Hadronic}{#splitline{p_{T}^{#gamma#gamma} < 45 GeV}{0.28 < BDT < 0.61}}");
    desc.ReplaceAll("reco1 b","#splitline{Hadronic}{#splitline{45 #leq p_{T}^{#gamma#gamma} < 80 GeV}{0.28 < BDT < 0.61}}");
    desc.ReplaceAll("reco2 b","#splitline{Hadronic}{#splitline{80 #leq p_{T}^{#gamma#gamma} < 120 GeV}{0.28 < BDT < 0.61}}");
    desc.ReplaceAll("reco3 b","#splitline{Hadronic}{#splitline{120 #leq p_{T}^{#gamma#gamma} < 200 GeV}{0.28 < BDT < 0.61}}");
    desc.ReplaceAll("reco4 b","#splitline{Hadronic}{#splitline{200 #leq p_{T}^{#gamma#gamma} < 350 GeV}{0.28 < BDT < 0.61}}");
    desc.ReplaceAll("reco0 a","#splitline{Hadronic}{#splitline{p_{T}^{#gamma#gamma} < 45 GeV}{BDT > 0.61}}");
    desc.ReplaceAll("reco1 a","#splitline{Hadronic}{#splitline{45 #leq p_{T}^{#gamma#gamma} < 80 GeV}{BDT > 0.61}}");
    desc.ReplaceAll("reco2 a","#splitline{Hadronic}{#splitline{80 #leq p_{T}^{#gamma#gamma} < 120 GeV}{BDT > 0.61}}");
    desc.ReplaceAll("reco3 a","#splitline{Hadronic}{#splitline{120 #leq p_{T}^{#gamma#gamma} < 200 GeV}{BDT > 0.61}}");
    desc.ReplaceAll("reco4 a","#splitline{Hadronic}{#splitline{200 #leq p_{T}^{#gamma#gamma} < 350 GeV}{BDT > 0.61}}");
    desc.ReplaceAll("reco5","#splitline{Hadronic}{p_{T}^{#gamma#gamma} > 350 GeV}");
  ...
  for (int i=0; i<chan->numTypes(); ++i) {
    ...
    win->var("ggHScale")->setVal(0);
    double sbevents_noggH = sbcatpdf->expectedEvents(*mass);    
    win->var("ggHScale")->setVal(1);
    double bevents = bcatpdf->expectedEvents(*mass);
    double sevents = sbevents - bevents;
    double sevents_noggH = sbevents_noggH - bevents;
    
    std::cout << "[INFO] number of events  sbevents  " << sbevents << " bevents " << bevents << " sevents " << std::endl;
 
    win->var("ggHScale")->setVal(0);
    TH1D *hsbtmp_noggH = (TH1D*)sbcatpdf->createHistogram("hsbtmp_noggH",*mass,Binning(3200));
    win->var("ggHScale")->setVal(1);
    ...
    TH1 *hstmp_noggH = new TH1D ( sbevents_noggH*(*hsbtmp_noggH) - bevents*(*hbtmp) );    
  ...
  TH1D *hsig_noggH = new TH1D("hsig_noggH","",3200,100.,180.);
  TH1D *hwsig = new TH1D("hwsig","",3200,100.,180.);
  TH1D *hwsig_noggH = new TH1D("hwsig_noggH","",3200,100.,180.);
  ...
  TH1D *hsigbkg_noggH = new TH1D("hsigbkg_noggH","",3200,100.,180.);
  TH1D *hwsigbkg = new TH1D("hwsigbkg","",3200,100.,180.);   
  TH1D *hwsigbkg_noggH = new TH1D("hwsigbkg_noggH","",3200,100.,180.);   
  ...
  TH1D *hsig_noggH_bkgplotfine = new TH1D("hsig_noggH_bkgplotfine","",nbinsfine,lowedge,highedge);
  TH1D *hwsig_noggH_bkgplotfine = new TH1D("hwsig_noggH_bkgplotfine","",nbinsfine,lowedge,highedge);
  
  std::vector<TH1D*> hbkgplots;
  std::vector<TH1D*> hsigplotsfine;
  std::vector<TH1D*> hsig_noggH_plotsfine;
  ...
  for (int i=0; i<chan->numTypes(); ++i) {
    ...
    std::cout << " name = " << sbcatpdf->GetName() << std::endl;
    std::cout << " S+B Event yields = " << sbevents << std::endl;
    std::cout << " S+B Event, noggH  yields = " << sbevents_noggH << std::endl;
    TH1D *hsbtmp = (TH1D*)sbcatpdf->createHistogram("hsbtmp",*mass,Binning(3200));
    win->var("ggHScale")->setVal(0);
    TH1D *hsbtmp_noggH = (TH1D*)sbcatpdf->createHistogram("hsbtmp_noggH",*mass,Binning(3200));
    win->var("ggHScale")->setVal(1);
    ...
    hsbtmp_noggH->Scale(1/hsbtmp_noggH->Integral());
    ...
    win->var("ggHScale")->setVal(0);
    TH1D *hs_noggH_bplotfinetmp = (TH1D*) sbcatpdf->createHistogram("hs_noggH_bplotfinetmp",*mass,Binning(nbinsfine,lowedge,highedge));
    win->var("ggHScale")->setVal(1);
    ...
    hs_noggH_bplotfinetmp->Scale(1/hs_noggH_bplotfinetmp->Integral());
    hbplotfinetmp->Scale(1/hbplotfinetmp->Integral());

    hsigbkg->Add(hsbtmp,sbevents);
    hsigbkg_noggH->Add(hsbtmp_noggH,sbevents_noggH);
    hwsigbkg->Add(hsbtmp,catweight*sbevents);
    hwsigbkg_noggH->Add(hsbtmp_noggH,catweight*sbevents_noggH);
    ...
    hsig_noggH_bkgplotfine->Add(hs_noggH_bplotfinetmp,sbevents_noggH);
    hwsig_noggH_bkgplotfine->Add(hs_noggH_bplotfinetmp,catweight*sbevents_noggH);    
    
    TH1 *hstmp = new TH1D ( sbevents*(*hsbtmp) - bevents*(*hbtmp) );
    hsig->Add(hstmp);
    
    TH1 *hstmp_noggH = new TH1D ( sbevents_noggH*(*hsbtmp_noggH) - bevents*(*hbtmp) );
    hsig_noggH->Add(hstmp_noggH);
    
    TH1 *hwstmp = new TH1D ( catweight*(sbevents*(*hsbtmp) - bevents*(*hbtmp)) );
    hwsig->Add(hwstmp);
    
    TH1 *hwstmp_noggH = new TH1D ( catweight*(sbevents*(*hsbtmp_noggH) - bevents*(*hbtmp)) );
    hwsig_noggH->Add(hwstmp_noggH);
    ...
    TH1D *hsig_noggH_plot_cat = new TH1D(TString::Format("hsig_noggH_plot_cat%s",catnames[i].Data()),"",3200,100.,180.);
    hsigplot_cat->Add(hstmp);
    hsig_noggH_plot_cat->Add(hstmp_noggH);
    hsigplotsfine.push_back(hsigplot_cat);
    hsig_noggH_plotsfine.push_back(hsig_noggH_plot_cat);
  ...
  hsig_noggH_plotsfine.push_back(hsig_noggH);
  ...
  hsigbkg_noggH->Scale(curvescale);
  ...
  hsig_noggH_bkgplotfine->Scale(curvescaleplotfine);
  ...
  for (int isig=0; isig<hsigplotsfine.size(); ++isig) {
    ...
    hsig_noggH_plotsfine[isig]->Scale(curvescale);
  }
  ...
  hsig_noggH_bkgplotfine->SetLineWidth(deflinewidth);
  ...
  hsigbkg_noggH->SetLineColor(kBlue);
  hsigbkg_noggH->SetLineWidth(deflinewidth);
  hwsigbkg->SetLineColor(kRed);
  hsigbkgplotfine->SetLineColor(kRed);
  hsig_noggH_bkgplotfine->SetLineColor(kBlue);
  hsig_noggH_bkgplotfine->SetLineWidth(deflinewidth);
  ...
  hbkgplotfine->SetLineColor(kBlack);
  ...
  hdummy->GetXaxis()->SetTitleSize(2.4*hdummy->GetXaxis()->GetTitleSize());
  hdummy->GetXaxis()->SetLabelSize(2.2*hdummy->GetXaxis()->GetLabelSize());
  hdummy->GetYaxis()->SetTitleSize(1.7*hdummy->GetYaxis()->GetTitleSize());
  hdummy->GetYaxis()->SetLabelSize(2.2*hdummy->GetYaxis()->GetLabelSize());
  ...
  for (int i=0; i<(chan->numTypes()+2); ++i) {
   ...
   catdatam = (RooDataSet*) sbcatpdf->generate(RooArgSet(*mass),catdatam->sumEntries());
...
//     pad2->SetBorderMode(0);
    ...
    std::cout << "[INFO] draw " << chan->getLabel() << " s and b pdfs" << std::endl; 
      catbpdf->plotOn(catplot,Normalization(catbpdf->expectedEvents(catdatam->get()),RooAbsPdf::NumEvent),LineColor(kBlack),LineStyle(kDashed),LineWidth(deflinewidth));  
      win->var("ggHScale")->setVal(0); 
      sbcatpdf->plotOn(catplot,Normalization(sbcatpdf->expectedEvents(catdatam->get()),RooAbsPdf::NumEvent),LineColor(kBlue),LineWidth(deflinewidth));      
      win->var("ggHScale")->setVal(1); 
    ...
    catplot->SetMaximum(1.8*catplot->GetMaximum());
    if (!drawZeroBins_) catplot->SetMinimum(1e-5);
    
    TLegend *leg2 = new TLegend(0.56,0.47,0.885,0.865);  
    leg2->AddEntry(plotdata,"Pseudo-data","PE");  
    leg2->AddEntry(hsigbkg,"S+B fit","L");  
    leg2->AddEntry(hsigbkg_noggH,"ttH + tH + B","L");  
    leg2->AddEntry(hbkg,"B","L");  
    ...
    lat2->SetTextSize(0.052);
    lat2->SetNDC();
    lat2->SetTextAlign(13);
    // lat2->DrawLatex(0.136,0.62,TString::Format("%s",catdesc.at(i).Data())); // Leptonic works ok 
    lat2->DrawLatex(0.16,0.85,"ttH + tH, H#rightarrow#gamma#gamma");
    lat2->DrawLatex(0.146,0.79,TString::Format("%s",catdesc.at(i).Data())); // Hadronic works ok 
    /*
    if (quoteMu_){
    lat2->DrawLatex(0.159,0.8,Form("m_{H}=%.2f GeV, #hat{#mu}=%.2f",MH->getVal(),r->getVal()));
    } else {
    lat2->DrawLatex(0.159,0.8,Form("m_{H}=%.2f GeV",MH->getVal()));
    }
    */
    //lat2->DrawLatex(0.56585,0.76+offset,"-"); // top
    ...
    lat3->SetNDC();
    lat3->SetTextSize(0.048);
    //lat3->DrawLatex(0.13,0.93,"#bf{CMS} #scale[0.75]{#it{Preliminary}}");
    lat3->DrawLatex(0.13,0.93,"#bf{CMS Phase-2} #it{Simulation Preliminary}"); // for the paper
    
    TLatex *mytext = new TLatex();
    mytext->SetTextSize(0.044);
    mytext->SetNDC();
    mytext->DrawLatex(0.68,0.93,Form(" %d ab^{-1} (14#scale[0.75]{ }TeV)",3));        
    ...
    hdummy->SetMaximum(int(hdatasub->GetHistogram()->GetMaximum())+3);
    hdummy->SetMinimum(int(hdatasub->GetHistogram()->GetMinimum())-3);
    ...
    TH1D *hsig_noggH_plotfine = hsig_noggH_plotsfine[i];
    hsig_noggH_plotfine->SetLineColor(kBlue);
    hsig_noggH_plotfine->SetLineWidth(deflinewidth);
    
    std::cout << "hsig int="<< hsigplotfine->Integral() <<std::endl;
    std::cout << "hsig no ggH int="<< hsig_noggH_plotfine->Integral()<<std::endl;
    hsig_noggH_plotfine->Draw("HISTSAME");
    ...
    lat4->DrawLatex(0.65,0.86,TString::Format("B subtracted"));
