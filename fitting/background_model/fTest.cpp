  ...
  RooWorkspace *inWS;
	if(isFlashgg_){
		if (isData_){
			...
			inWS = (RooWorkspace*)inFile->Get("tagsDumper/cms_hgg_workspace");
		}
	} else {
	...
	for (int cat=startingCategory; cat<ncats; cat++){
		...
		if (isData_) {
    dataFull = (RooDataSet*)inWS->data(Form("allBackground_13TeV_%s",catname.c_str()));
    ...
    dataFull->add(*dataFull0->get(),1.0);
		...
		if (verbose) std::cout << "[INFO] opened data for  "  << Form("allBackground_13TeV_%s",catname.c_str()) <<" - " << dataFull <<std::endl;
    }
		else 
    {dataFull = (RooDataSet*)inWS->data(Form("allBackground_13TeV_%s",catname.c_str()));
		if (verbose) std::cout << "[INFO] opened data for  "  << Form("allBackground_13TeV_%s",catname.c_str()) <<" - " << dataFull <<std::endl;
        ...
        bkgPdf->Print();
			...
			if (saveMultiPdf){
				...
				while (prob<upperEnvThreshold){
					RooAbsPdf *bkgPdf = getPdf(pdfsModel,*funcType,order,Form("env_pdf_%s_%s",flashggCats_[cat].c_str(),ext.c_str()));
					if (!bkgPdf ){
						// assume this order is not allowed
