...
variables = { 
	   ...
	   ,"Nleptons":["N leptons",8,0,8,True,0,[],[]] 
	   ...
	   ,"lep_pT":["p_{T}^{l} (GeV)",40,0,400,True,0,[],[]] 
	   ,"lep_eta":["|#eta|^{l} (GeV)",15,0,5,True,0,[],[]] 
	   ,"bdt_class":["Leptonic BDT output",12,-0.8,1,True,0,[],[]] 
	   ,"mindphi_gg_l":["#Delta#phi(#gamma#gamma,l^{1})",50,0,5,True,0,[],[]] 
...
fout    = ROOT.TFile("bdt_trees_leptonic.root","RECREATE")
fout.cd()
...
oTree_s.Branch("Nleptons",	b_Nlep,"Nleptons/F")
...
oTree_s.Branch("lep_pt",	b_lep_pt,"lep_pt/F")
oTree_s.Branch("lep_eta",	b_lep_eta,"lep_eta/F")
oTree_s.Branch("out_BDTG",		b_BDT,"out_BDTG/F")
oTree_s.Branch("mgg",		b_mgg,"mgg/F")
oTree_s.Branch("mindphi_gg_l",		b_mindphi_gg_l,"mindphi_gg_l/F")


oTree_b.Branch("weight",b_weight,"weight/F")
oTree_b.Branch("Njets",b_Njets,"Njets/F")
oTree_b.Branch("Nleptons",	b_Nlep,"Nleptons/F")
...
oTree_b.Branch("lep_pt",	b_lep_pt,"lep_pt/F")
oTree_b.Branch("lep_eta",	b_lep_eta,"lep_eta/F")
oTree_b.Branch("out_BDTG",b_BDT,"out_BDTG/F")
oTree_b.Branch("mgg",b_mgg,"mgg/F")
oTree_b.Branch("mindphi_gg_l",		b_mindphi_gg_l,"mindphi_gg_l/F")
...
if runBDT : tmvaReader_.BookMVA("BDTG","weights_leptonic/TMVAClassification_BDTG.weights.xml")
...
def doAnalysis(tr,entry,i,w):
     ...
     variables["Nleptons"][entry][i].Fill(tr.Nleptons,w)
     ...
     if tr.jet3_pT < 0:  variables["jet3_eta"][entry][i].Fill(-999,w)
     else: variables["jet3_eta"][entry][i].Fill(abs(tr.jet3_eta),w)
     
     variables["lep_pT"][entry][i].Fill(tr.lep_pT,w)
     variables["lep_eta"][entry][i].Fill(abs(tr.lep_eta),w)

     variables["bdt_class"][entry][i].Fill(bdtg,w)
     variables["mindphi_gg_l"][entry][i].Fill(mindelphi,w)
