...
for syst_type in ['THU_QCDscale_inc','THU_PDF_inc','THU_alpha_inc']:
  ...
  syst_line_VH = '%s_VH lnN '%syst_type
  ...
  for reco_idx in range(0,n_cats):
    ...
    for proc in dat_procs:
      if( 'VH' in proc ):
        if syst_type == 'THU_QCDscale_inc': syst_line_VH += '1.015/0.987 '
        elif syst_type == 'THU_PDF_inc': syst_line_VH += '1.011/0.989 '
        elif syst_type == 'THU_alpha_inc': syst_line_VH += '1.007/0.993 '
      else: syst_line_VH += '- '
