...
graphs = []
for g in ['nominal','exp_only','stat_only']:
  gr = ROOT.TGraph()
  gr.SetName("gr_%s_%s"%(x,g))
  if( g == 'nominal' ): gr.SetLineColor(1)
  elif( g == 'exp_only' ): gr.SetLineColor(2)
  elif( g == 'stat_only' ): gr.SetLineColor(1)
  gr.SetLineWidth(2)
  if( g == 'stat_only' ): gr.SetLineStyle(2)
  graphs.append( gr ) 
...
for gr_idx in range( len(graphs) ):
  ...
  elif gr_idx == 1: f_in = ROOT.TFile("combineJobs_combined_preapp/klambda_systematics/k_lambda_ExpSystOnly/k_lambda_ExpSystOnly.root")
  elif gr_idx == 2: f_in = ROOT.TFile("combineJobs_combined_preapp/klambda_systematics/k_lambda_StatOnly/k_lambda_StatOnly.root")
  tree = f_in.Get('limit')
...
leg1.SetFillColor(0)
leg1.SetLineColor(0)
leg1.AddEntry("gr_k_lambda_nominal","w/ YR18 syst. uncert.","L")
leg1.AddEntry("gr_k_lambda_exp_only","Stat. + exp. syst. uncert.","L")
leg1.AddEntry("gr_k_lambda_stat_only","Stat. uncert. only","L")
leg1.Draw("Same")
...
#outf.cd()
canv.Write()
canv.Print("klambda_scan_systematics.pdf")
