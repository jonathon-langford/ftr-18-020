...
graphs = []
for g in ['1ab','2ab','3ab']:
  gr = ROOT.TGraph()
  gr.SetName("gr_%s_%s"%(x,g))
  if( g == '1ab' ): gr.SetLineColor(4)
  elif( g == '2ab' ): gr.SetLineColor(8)
  elif( g == '3ab' ): gr.SetLineColor(1)
  gr.SetLineWidth(2)
  graphs.append( gr ) 
...
for gr_idx in range( len(graphs) ):
  ...
  #Read in file
  #tree = 0
  if gr_idx == 0: f_in = ROOT.TFile("combineJobs_combined_preapp/klambda_luminosities/k_lambda_1ab/k_lambda_1ab.root")
  elif gr_idx == 1: f_in = ROOT.TFile("combineJobs_combined_preapp/klambda_luminosities/k_lambda_2ab/k_lambda_2ab.root")
  elif gr_idx == 2: f_in = ROOT.TFile("combineJobs_combined_preapp/k_lambda/k_lambda.root")
  tree = f_in.Get('limit')
...
lat.DrawLatex(0.72,0.92,"          (14#scale[0.75]{ }TeV)")
...
#Legend
leg1 = ROOT.TLegend(0.35,0.542373,0.67,0.88)
leg1.SetFillColor(0)
leg1.SetLineColor(0)
leg1.SetTextSize(0.05)
leg1.AddEntry("gr_k_lambda_3ab","3 ab^{-1}","L")
leg1.AddEntry("gr_k_lambda_2ab","2 ab^{-1}","L")
leg1.AddEntry("gr_k_lambda_1ab","1 ab^{-1}","L")
leg1.Draw("Same")
...
#outf.cd()
canv.Write()
canv.Print("klambda_scan_luminosities.pdf")
