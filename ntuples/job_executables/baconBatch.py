...
def getFilesJob(dirin,job,tnjobs):
  ...
  for dir in alldirs:
    if '/store/' in dir :
      #If files at fermilab i.e. diphoton
      if '/snowmass/' in dir: infiles.extend(makeCaFiles(dir,options.blacklist,tnjobs,job,site="fnal.gov")) 
      #If files at desy i.e. singlelepfromtbar
      elif 'SingleLeptFromTbar' in dir: infiles.extend(makeCaFiles(dir,options.blacklist,tnjobs,job,site="desy"))
      elif 'TTGG_0Jets' in dir: infiles.extend(makeCaFiles(dir,options.blacklist,tnjobs,job,site="desy"))
      #else if standard cern eos
      else: infiles.extend(makeCaFiles(dir,options.blacklist,tnjobs,job))
