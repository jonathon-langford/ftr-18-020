...
if( signal_type == "ttH" ): filename = "ttHHad_tri_ntuples_syst/" + syst_type + "/ttH/ttH_" + nOut  + "_" + syst_direction +".root"
elif( signal_type == "ggH" ): filename = "ttHHad_tri_ntuples_syst/" + syst_type + "/ggH/ggH_" + nOut  + "_" + syst_direction +".root"
elif( signal_type == "THQ" ): filename = "ttHHad_tri_ntuples_syst/" + syst_type + "/THQ/THQ_" + nOut  + "_" + syst_direction + ".root"
elif( signal_type == "THW" ): filename = "ttHHad_tri_ntuples_syst/" + syst_type + "/THW/THW_" + nOut  + "_" + syst_direction + ".root"
elif( signal_type == "VH" ): filename = "ttHHad_tri_ntuples_syst/" + syst_type + "/VH/VH_" + nOut  + "_" + syst_direction + ".root"
  ...
  if( photons_pass ) & ( leptonVeto_pass ):
    if branchJet.GetEntries() > 0:
      ...
      if( len(jets)+len(bjets) >= 3 ) & ( len(bjets) >= 1 ):
        ...
        jets.sort( key=lambda J: (J.PT*alpha), reverse=True )
        bjets.sort( key=lambda bJ: (bJ.PT*alpha), reverse=True )
...
#	EVENTS PASSING SELECTION
  if( hasHiggs ) & (photons_pass) & (leptonVeto_pass) & (top_pass):
    ...
    if "THU_ggH" in syst_type:
      ...
      #print "Entry:%g"%entry
      #print "  Syst type: %s, Syst direction: %s"%(syst_type,syst_direction)
      #if( syst_direction == "up" ): print "  ggH scale factors: %s"%qcd_ggH_uncert_2017(Njets30,higgs_gen.PT,1.0)
      #else: print "  ggH scale factors: %s"%qcd_ggH_uncert_2017(Njets30,higgs_gen.PT,-1.0)
      #print "  Re-weighting = %5.4f"%ggH_modifier
    ...
    if( len(merged_jets) > 3 ):
      jet4_pT[0] = merged_jets[3].PT*alphas[3]
      ...
      if( len(merged_jets) > 4 ):
        jet5_pT[0] = merged_jets[4].PT*alphas[4]
        ...
        if( len(merged_jets) > 5 ):
          jet6_pT[0] = merged_jets[5].PT*alphas[5]
